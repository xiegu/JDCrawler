...
class JDItemCommentCrawler(object):
	...
	def run(self):
		...
		query = CatItemSqlHelper().select()[30000:30002]
		itemIdList = [i[0] for i in query]
		print 'totally %d items' %len(itemIdList)
		self.commentThread(itemIdList)
		print 'totally %d items in COMMENT' %len(COMMENT)
		commentIdList = [s['id'] for s in COMMENT]
		for i in range(len(COMMENT)):
			j = itemIdList.index(commentIdList[i])
			COMMENT[i]['category'] = query[j][1]
		for i in COMMENT:
			if i is not None:
				self.queue2.put(i)
	...
	def commentCrawler(self, parser, itemId, pageNum = 100):
		...
		print '%s done' %itemId
		COMMENT.extend(itemList)
		print 'COMMENT length %d' %len(COMMENT)